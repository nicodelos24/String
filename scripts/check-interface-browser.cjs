...
(async()=>{
  ...
  try {
    ...
    await evaluate("document.querySelector('#section-list').scrollIntoView({block:'center'});");
    await delay(100);
    const sectionDrag=await evaluate("(()=>{const a=document.querySelector('.section-grip').getBoundingClientRect(),b=document.querySelectorAll('.section-card')[1].getBoundingClientRect();return {x:a.x+a.width/2,y:a.y+a.height/2,toX:b.x+b.width/2,toY:b.y+b.height/2};})()");
    await send('Input.dispatchMouseEvent',{type:'mousePressed',x:sectionDrag.x,y:sectionDrag.y,button:'left',clickCount:1});
    await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:sectionDrag.toX,y:sectionDrag.toY,button:'left',buttons:1});
    ...
    assert.equal(await evaluate("document.querySelector('#progression .progression-card').dataset.index"),'1');
    assert(await evaluate("document.querySelector('.progression-playback').contains(document.querySelector('#player-status')) && !document.querySelector('.chord-player').contains(document.querySelector('#player-status'))"));
    await evaluate("window.dispatchEvent(new Event('traste:load-song'));document.querySelector('.sections-panel').open=false;");
