...
También que si está todo pausado y toco una tarjeta por ejemplo, se ponga play. esto haría más intuitivo todo.
  ...
  - Implementado: el botón **Tempo** y su campo BPM salen de la fila del mástil (`.progression-quick-player`) y pasan a una fila propia, `.progression-tempo`, justo debajo de las tarjetas y del texto de estado (`player-status`), dentro de `.progression-workspace`. Así el tempo queda a mano de quien está trabajando con la progresión, sin bajar hasta el mástil, y se ve sin desplazar la página.
  ...
  - Verificado en Chromium sobre `npm start`: la fila del tempo queda bajo las tarjetas y el estado, sin desbordar a 420, 700 y 1400 px; un clic en la tarjeta arranca y tres clics seguidos no cambian el BPM; el doble clic real duplica (5 tarjetas); tres clics reales en el botón Tempo dan ~116 BPM y los aplica con la música sonando. Pendiente tu prueba real, sobre todo el doble clic y marcar el tempo con la guitarra.
