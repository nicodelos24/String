...
*Quisiera implementar que en el mástil de guitarra o bajo también se pueda tocar con las teclas de la pc, con los mismos controles que el teclado
  - Implementado: mientras la sección «Teclado» está cerrada, las mismas teclas del piano (`a s d f g h j` = do–si, `w e t y u` = sostenidos, `Shift izq/der` para bajar/subir octava) tocan el mástil en vez del piano: la nota pulsada se suena con el sintetizador y se remarca en las posiciones de guitarra/bajo en violeta (`#6c5ce7`, clase `.keyboard-live`), distinto del ámbar del micrófono (`.live`). Al soltar la tecla deja de sonar y se quita el remarcado (soporta polifonía). Al abrir la sección «Teclado», las teclas vuelven al piano y el mástil queda sin el remarcado violeta.
  - `npm test` → 159/159 (nuevo test unitario de `mastilKeyToggle`). Verificado en navegador headless: con el teclado cerrado `a` marca las 5 posiciones de C4 en violeta y soltarla las limpia (polifonía con `a`+`s`, shift → C3), al abrir el piano ya no marca el mástil, y en bajo solo marca posiciones reales. Pendiente tu revisión del color, que suene el sintetizador en este modo y de la alineación.