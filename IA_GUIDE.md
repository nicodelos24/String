...
*Quisiera implementar que en el mástil de guitarra o bajo también se pueda tocar con las teclas de la pc, con los mismos controles que el teclado
  ...
  - Teclas ampliadas (letras por idioma del teclado, con respaldo por posición física `event.code`): naturales `z x c v b n m` (octava grave C3–B3), `a s d f g h j k l ñ {` (C4–F5, `ñ`=E5 y `{`=F5 para teclado español en lugar de `;`), sostenidos `w e t y u o p ´ +` (C#–G# de C#4 a G#5; `´` es el acento muerto, se resuelve por su posición).
