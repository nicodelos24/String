...
También que si está todo pausado y toco una tarjeta por ejemplo, se ponga play. esto haría más intuitivo todo.
  ...
  - `npm test` → 224/224 (`tests/progression-interactions.test.cjs` con el límite de 250 ms: 200 ms duplica, 250 ms ya no, y tras duplicar el siguiente clic empieza de nuevo).
  - Verificado en Chromium sobre `npm start`: el orden en la columna es «acorde en una fila» → **Tempo** → «Seguir acorde MIDI» → «?», con el interruptor intacto; el hueco que mide la app decide, y con dos clics medidos a 267-288 ms no duplica mientras que los pares por debajo de 250 ms sí; tres clics a 300 ms y a 400 ms, cada uno ya procesado, no duplican; el botón Tempo sigue estimando y aplicando en marcha. Pendiente tu prueba real, sobre todo el doble clic y marcar el tempo con la guitarra.
