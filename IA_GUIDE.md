...
*Quisiera implementar que en el mástil de guitarra o bajo también se pueda tocar con las teclas de la pc, con los mismos controles que el teclado
  ...
  - Switch «Por cuerdas» (debajo del candado del mástil): activado, con el «Teclado» cerrado cada fila del teclado toca una sola cuerda cromáticamente (cada tecla sube un semitono en esa cuerda). En guitarra: `z x c v b n m , . -` = cuerda E (z=mi, x=fa, c=fa#, v=sol…), `a s d f g h j k l ñ {` = cuerda D, `q w e r t y u i o p ´ +` = cuerda B, `1 2 3 4 5 6 7 8 9 0 - =` = cuerda E aguda. En bajo: las cuatro cuerdas E, A, D, G a esas filas. Los shift de octava se ignoran en este modo (cada fila ya nace anclada a su cuerda abierta). `npm test` → 165/165; verificado en headless (x=fa, v=sol, a=re, q=si, 1=mi aguda). Nota: el traste 0/cuerdas al aire se renderizan al llegar al clavijero, así que `z`=mi muestra su remarcado recién al estar ahí la vista.