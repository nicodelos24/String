...
*Quisiera implementar que en el mástil de guitarra o bajo también se pueda tocar con las teclas de la pc, con los mismos controles que el teclado
  ...
  - **Acorde en vivo desde el teclado**: switch «Acorde en vivo» junto al candado (pie del mástil). Activado con el «Teclado» cerrado, al mantener 3 o más notas con las teclas de la PC el mástil muestra en vivo la escala del acorde que forman y la tarjeta «ACORDE EN VIVO» lo nombra («Notas mantenidas · Enter para añadir»); `Enter` (o el botón ＋ Añadir) agrega ese acorde a las tarjetas de la progresión. Con el switch apagado, las teclas tocan el mástil sin cambiar la escala. Reconocimiento por `chordFromNotes` usando las mismas plantillas que el micrófono (ej.: `a d g` = Do mayor, `s g j` = Re mayor, `a d g j` = Do mayor 7… con raíz según las 3+ notas). El micrófono cede la tarjeta en vivo cuando el teclado la está usando. `npm test` → 164/164.