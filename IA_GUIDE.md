...
*Quisiera implementar que en el mástil de guitarra o bajo también se pueda tocar con las teclas de la pc, con los mismos controles que el teclado
  - Implementado: al tocar una tecla física o una tecla del piano con el puntero, las posiciones del mástil y cuerdas al aire con ese mismo MIDI se remarcan en violeta (`#6c5ce7`, clase `.keyboard-live`), distinto del ámbar del micrófono (`.live`), para guitarra o bajo según el instrumento activo. Al soltar la tecla o cerrar la sección se quita el remarcado; si el mástil se reconstruye (acorde, modo, instrumento) con notas aún sonando, el resaltado se reaplica.
  - `npm test` → 159/159 (nuevo test unitario de `mastilKeyToggle`). Verificado en navegador headless: `a` marca las 5 posiciones de C4 en guitarra y soltarla las limpia, lo mismo con el puntero, y en bajo solo marca las posiciones reales de ese MIDI. Pendiente tu revisión del color y la alineación visual.